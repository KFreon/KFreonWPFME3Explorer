...
namespace KFreonLibME.PCCObjects
{
    ...
    public abstract class AbstractPCCObject : IDisposable
    {
        ...
        public static AbstractPCCObject Create(string file, int gameversion, string pathbiogame)
        {
            ...
            {
                throw new InvalidOperationException("GameVersion parameter must be between 1 and 3.");
            }
        ...
        public static AbstractPCCObject Create(string file, MemoryTributary stream, int gameversion, string pathbiogame)
        {
            AbstractPCCObject pcc;
            if (gameversion == 1)
            ...
            {
                throw new InvalidOperationException("GameVersion must be between 1 and 3.");
                return null;
            }
        ...
        public MemoryTributary ListStream { get; set; }
        public MemoryTributary DataStream { get; set; }
        ...
        protected MemoryTributary tempStream { get; set; }
        ...
        protected AbstractPCCObject(string path)
        {
            ...
            tempStream = new MemoryTributary();
        ...
        protected virtual void LoadFromStream(MemoryTributary tempStream)
        {
        ...
        public virtual void ReadNames(MemoryTributary fs)  // ME1 and 2
        {
        ...
        public virtual void SaveToFile(string path)
        {
            ...
            foreach (String name in Names)
            {
                if (name != null)
                {
                    ...
                    ListStream.WriteValueS32(name.Length + 1);   // +1 for ME2 at least
                    ListStream.WriteString(name);
                }
                else
                    ListStream.WriteValueS32(1);
                ListStream.WriteByte(0);
                ListStream.WriteValueS32(-14);

                if (GameVersion == 1)
                    ListStream.WriteValueS32(5);  // KFreon: TESTING
            }
        ...
        internal List<Exception> Scan(string ExecFolder, TreeDB Tree)
        {
            byte[] imgData = null;
            METexture2D tex2D = null;
            ImageInfo info = null;
            string thumbName = null;
            string GameThumbFolder = null;
            ...
            for (int expID = 0; expID < Exports.Count; expID++)  // KFreon: Use explicit Count in case header count is different
            {
                ...
                if (Exports[expID].ValidTextureClass())
                {
                    ...
                    thumbName = tex2D.texName + "_" + hash.ToString() + ".jpg";
                    DebugOutput.PrintLn("Generating thumbnail: " + thumbName);
                    GameThumbFolder = "ME" + GameVersion.ToString() + "ThumbnailCache\\";
                    ThumbnailPath = Path.Combine(ExecFolder, "ThumbnailCaches\\", GameThumbFolder , thumbName);


                    // KFreon: Create Tree entry for texture
                    TreeTexInfo tex = new TreeTexInfo(tex2D, expID, hash, pccFileName, info, GameVersion, PathBIOGame, true) { ThumbnailPath = ThumbnailPath };
                    bool Added = Tree.AddTex(tex, Path.GetFileNameWithoutExtension(pccFileName).ToUpperInvariant());

                    if (Added && !File.Exists(ThumbnailPath))
                    {
                        using (ResILImage img = new ResILImage(imgData, ResIL.Unmanaged.ImageType.Dds))
                        {
                            // KFreon: Resize image to thumbnail sizes (ResIL resize currently inoperable)
                            /*if (!img.Resize(64, 64))
                                throw new Exception("Image resize failed!");*/
                            using (MemoryTributary imgstream = new MemoryTributary())
                            {
                                if (img.ConvertAndSave(ResIL.Unmanaged.ImageType.Jpg, imgstream))
                                {
                                    int decodeWidth = img.Width > img.Height ? 128 : 0;
                                    int decodeHeight = img.Width > img.Height ? 0 : 128;


                                    BitmapImage bitmap = UsefulThings.WPF.Images.CreateWPFBitmap(imgstream, decodeWidth, decodeHeight);
                                    UsefulThings.WPF.Images.SaveWPFBitmapToDiskAsJPG(bitmap, ThumbnailPath);
                                }
                                else
