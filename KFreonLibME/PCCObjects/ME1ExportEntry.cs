    ...
    public class ME1ExportEntry : AbstractExportEntry
    {
        ...
        private void MoveNames()
        {
            ...
            foreach (string name in pccRef.Names)
            {
                if (name != null)
                {
                    pccRef.ListStream.WriteValueS32(name.Length + 1);
                    pccRef.ListStream.WriteString(name);
                    pccRef.ListStream.WriteByte(0);
                    pccRef.ListStream.WriteValueS32(0);
                    pccRef.ListStream.WriteValueS32(458768);
                }
            }
        }
    }
