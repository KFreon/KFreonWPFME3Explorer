...
namespace KFreonLibME.Textures
{
    public class METexture2D
    {
        ...
        public METexture2D(string texname, ICollection<PCCEntry> pccs, string pathbio, int gameversion, uint hash, TextureFormat format) : this()
        {
            texFormat = format;
            texName = texname;
        ...
        public METexture2D(string texname, List<string> pccs, List<int> ExpIDs, string pathBIOGame, int gameversion, TextureFormat format, uint hash = 0, String listname = null) : this(texname, PCCEntry.PopulatePCCEntries(pccs, ExpIDs), pathBIOGame, gameversion, hash, format)
        {
            ListName = listname;
        }
        ...
        public void PopulateFromPCC(AbstractPCCObject pcc, int pccExpID)
        {
            if (pcc.isExport(pccExpID))
            {
                ...
                if (exp.ClassName == class1 || exp.ClassName == class2 || exp.ClassName == class3)
                {
                    ...
                    if (!PCCs.Contains(t => t.File.Contains(pccFileName)))
                    ...
                    for (int i = 0; i < tempProperties.Count; i++)
                    {
                        ...
                        {
                            case "Format": texFormat = Textures.Methods.ParseTextureFormat((GameVersion == 3 ? property.Value.String2.Substring(3) : property.Value.StringValue)); break;
            ...
            MemoryTributary dataStream = new MemoryTributary(imageData);  // FG: we will move forward with the memorystream (we are reading an export entry for a texture object data inside the pcc)
            imgList = new List<ImageInfo>();
            if (GameVersion != 3)
                dataStream.ReadValueU32(); //Current position in pcc
            Mips = (int)dataStream.ReadValueU32();                       // FG: 1st int32 (4 bytes / 32bits) is number of mipmaps
            int count = Mips;
            ...
            while (dataStream.Position < dataStream.Length && count > 0)
            {
                ...
                imgInfo.storageType = (PCCStorageType)dataStream.ReadValueS32();       // FG: 2nd int32 storage type (see storage types above in enum_struct)
        ...
        /// </summary>
        /// <param name="size">Size of image to extract (mipmap). Either size parameter, but not both.</param>
        /// <param name="imgSize">Size of image to extrac (mipmap). Either size parameter, but not both.</param>
        /// <returns>Image data.</returns>
        public byte[] GetRawImageData(int size = -1, ImageSize imgSize = null, bool DirectFromGameFiles = false)
        {
            byte[] imgdata = null;

            if (size == -1 && imgSize == null)
                imgdata = extractMaxImage(true);
            else
            {
                ImageSize tes = imgSize;  // KFreon: Use specified if available
                if (imgSize == null)
                {
                    if (imgList.Count != 1)
                ...
                imgdata = DirectFromGameFiles ? GetRawDataFromGameFile(tes, true) : extractImage(tes, true);
            }
            return imgdata;
        }
        ...
        public byte[] extractMaxImage(bool NoOutput, string archiveDir = null, string fileName = null)
        {
            ...
            return extractImage(imgList.Find(img => img.ImgSize == maxImgSize), NoOutput, archiveDir, fileName);
        }
        ...
        /// </summary>
        /// <param name="imgInfo">Information on desired image.</param>
        /// <param name="NoOutput">False = Save to disk, True = save to byte[].</param>
        /// <param name="archiveDir">Path to TFC Cache. Leave null for automatic detection.</param>
        /// <param name="fileName">Filename to save extracted image to. Only valid if NoOutput = false.</param>
        /// <returns>Image data</returns>
        public byte[] extractImage(ImageInfo imgInfo, bool NoOutput, string archiveDir = null, string fileName = null)
        {
            ...
            byte[] imgBuffer = GetRawDataFromGameFile(imgInfo, NoOutput, archiveDir, fileName);

            // KFreon: Process raw data into DDS format
            byte[] saveImg = imgBuffer;  /*ProcessRawImageData(imgInfo.ImgSize, imgBuffer, fileName);

            // KFreon: Save image to disk if asked
            if (!NoOutput)
        ...
        public byte[] ProcessRawImageData(ImageSize imgSize, byte[] imgBuffer, string fileName = null)
        {
            ImageFile imgFile = new DDS(fileName, imgSize, texFormat.ToString(), imgBuffer);
            return imgFile.ToArray();
        }

        public byte[] GetRawDataFromGameFile(ImageSize imgSize, bool NoOutput, string archiveDir = null, string fileName = null)
        {
            byte[] retval;
            if (imgList.Exists(img => img.ImgSize == imgSize))
                retval = GetRawDataFromGameFile(imgList.Find(img => img.ImgSize == imgSize), NoOutput, archiveDir, fileName);
            else
                throw new FileNotFoundException("Image with resolution " + imgSize + " not found");
            return retval;
        }

        public byte[] GetRawDataFromGameFile(ImageInfo imgInfo, bool NoOutput, string archiveDir = null, string fileName = null)
        {
            if (fileName == null)
        ...
        /// Extracts image from file.
        /// </summary>
        /// <param name="imgSize">Size of mipmap to extract.</param>
        /// <param name="NoOutput">True = save to byte[], false = save to file.</param>
        /// <param name="archiveDir">Path to TFC cache. Leave null for automatic detection.</param>
        /// <param name="fileName">Path to save extracted image. Valid only if NoOutput = false.</param>
        /// <returns></returns>
        ...
        private void replaceImage(string strImgSize, ImageFile imgFile, string archiveDir)
        {
            ...
            TextureFormat imgFileFormat = KFreonLibME.Textures.Methods.ParseTextureFormat(imgFile.format);
        ...
        {
            ImageFile imgFile = KFreonLibME.Textures.Creation.LoadAKImageFile(imgData);
        ...
        public void OneImageToRuleThemAll(string archiveDir, byte[] imgData)
        {
            ...
            else
            {
                ...
                for (int i = imgMipMap.imageList.Count - 1; i >= 0; i--)
                {
                    ...
                    TextureFormat newImageFileFormat = KFreonLibME.Textures.Methods.ParseTextureFormat(newImageFile.format);
        ...
        public void CopyImgList(METexture2D inTex, AbstractPCCObject pcc)
        {
            ...
            for (int i = 0; i < tempProperties.Count; i++)
            {
                ...
                {
                    case "Format":
                        texFormat = Textures.Methods.ParseTextureFormat(pcc.Names[property.Value.IntValue].Substring(3));
                        break;
        ...
        public string GetDetailsAsString()
        {
            ...
            sb.AppendLine("Texmod Hash:  " + Textures.Methods.FormatTexmodHashAsString(Hash));
        ...
        public void MoveCaches(string cookedPath, string NewCache)
        {
            ...
            using (FileStream newCache = new FileStream(Path.Combine(cookedPath, NewCache), FileMode.Open, FileAccess.Read))
            {
                ...
                for (int i = 0; i < (GameVersion == 3 ? 16 : 4); i++)
                {
                    ...
                    tempVal.IntValue = GameVersion == 3 ? newCache.ReadByte() : newCache.ReadValueS32();
        ...
        public uint GenerateHash(ImageInfo info, out byte[] imgData)
        {
            ...
            imgData = GetRawImageData(imgSize: info.ImgSize, DirectFromGameFiles: true);
            if (imgData == null)
                return 0;
            ...
            {
                if (texFormat != TextureFormat.ThreeDC)
            ...
            imgData = ProcessRawImageData(info.ImgSize, imgData);
