...
namespace ME3Explorer.Tools
{
    class DLCHelper
    {
        ...
        {
            string path = KFreonLibME.MEDirectories.MEDirectories.GetBIOGameProperty(3);
            if (path == null)
        ...
        public static void RepackDLCs(bool MakeBackup = true)
        {
            ...
            for (int i = 0; i < DLCs.Length; i++)
            {
                ...
                editor.Execute(Path.ChangeExtension(dlcbase.fileName, ".new"), null, KFreonLibME.Misc.Methods.GetNumThreads());
                ...
                DebugOutput.PrintLn("Finished updating " + commonname);
            }
