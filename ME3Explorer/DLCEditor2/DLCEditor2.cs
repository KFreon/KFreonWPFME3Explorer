    ...
    public partial class DLCEditor2 : Form
    {
        ...
        private void createReplaceModJobToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (d.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                mj.SetFileAsData(new FileInfo(d.FileName));  // KFreon: NEW method to use file on disk instead of duplicating

                mj.Name = "DLC File Replacement in DLC \"" + file + "\" with File #" + n + " with " + mj.Length + " bytes of data";
        ...
        {
            if (DLC == null || DLC.Files == null || DLC.Files.Length == 0)
                return;
            string result = Microsoft.VisualBasic.Interaction.InputBox("Please enter pattern for unpacking, keep default to unpack all", "ME3 Explorer", "pcc; tfc; afc; cnd; tlk; bin", 0, 0);
            if (result == "")
                return;
            result = result.Trim();
            if (result.EndsWith(";"))
            ...
            for (int i = 0; i < DLC.Files.Length; i++)
            {
                ...
                for (int j = 0; j < patt.Length; j++)
                    if (DLCpath.ToLower().EndsWith(patt[j].Trim().ToLower()) && patt[j].Trim().ToLower() != "" && !DLCpath.ToLower().EndsWith("pcconsoletoc.bin") && !DLCpath.ToLower().EndsWith("mount.dlc"))
                    {
                        ...
                        MemoryTributary m = DLC.DecompressEntry(i);
                        File.WriteAllBytes(outpath, m.ToArray());
                        Indexes.Add(i);
