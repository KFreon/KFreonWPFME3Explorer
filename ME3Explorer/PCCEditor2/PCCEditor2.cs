...
namespace ME3Explorer
{
    public partial class PCCEditor2 : Form
    {
        ...
        {
            int n = -1;
            if (CurrentView == 3 && treeView1.SelectedNode != null)
                    n = Convert.ToInt32(treeView1.SelectedNode.Name);
            if (CurrentView == 2)
                n = listBox1.SelectedIndex;
            return n;
        ...
        private void createBinaryReplaceJobFromFileToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!IsFromDLC)
            {
                ...
                if (d.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    mj.SetFileAsData(new FileInfo(d.FileName));

                    mj.Name = "Binary Replacement for file \"" + currfile + "\" in Object #" + n + " with " + mj.Length + " bytes of data";
            ...
            else
            {
                ...
                if (d.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    mj.Name = "Binary Replacement for file \"" + inDLCFilename + "\" in Object #" + n + " with " + mj.Length + " bytes of data";
                    string loc = Path.GetDirectoryName(Application.ExecutablePath);
        ...
        private void createBinaryReplaceJobFromObjectToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (!IsFromDLC)
            {
                KFreonLibME.Scripting.ModMaker.ModJob mj = KFreonLibME.Scripting.ModMakerHelper.GenerateMeshModJob(null, n, pcc.pccFileName, CopyArray(pcc.Exports[n].Data));
                KFreonLibME.Scripting.ModMakerHelper.JobList.Add(mj);
                MessageBox.Show("Done");
            }
            else
            ...
            else
            {
                using (OpenFileDialog ofd = new OpenFileDialog())
                {
                    ...
                    if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        METexture2D tex2D = new METexture2D(temp, n, path);

                        byte[] imgData = UsefulThings.General.GetExternalData(ofd.FileName);
                        if (imgData != null)
                        {
                            tex2D.OneImageToRuleThemAll(path, imgData);
                            AbstractExportEntry expEntry = temp.Exports[tex2D.pccExpIdx];
                            expEntry.Data = tex2D.ToArray(temp, expEntry.DataOffset);
            ...
            else
            {
                using (OpenFileDialog ofd = new OpenFileDialog())
                {
                    ...
                    if (ofd.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                    {
                        ...
                        ImageFile im = KFreonLibME.Textures.Creation.LoadAKImageFile(null, ofd.FileName);
                        tex2D.replaceImage(test, im, path);

                        AbstractExportEntry expEntry = temp.Exports[tex2D.pccExpIdx];
                        expEntry.Data = tex2D.ToArray(temp, false, expEntry.DataOffset);
                        temp.SaveToFile(temp.pccFileName);
                    }
                }
            }
