    ...
    public partial class CloneDialog2 : Form
    {
		...
		private void saveCloneToolStripMenuItem_Click(object sender, EventArgs e) {
            ...
            if (pcc.pccFileName != null)
            {
                ...
                if (path.StartsWith(Path.Combine(KFreonLibME.Misc.Methods.GetNumThreads().ToString(), "CookedPCConsole")) && pcc.Exports[pcc.Exports.Count - 1].ClassName == "Texture2D" || pcc.Exports[pcc.Exports.Count - 1].ClassName == "LightMapTexture2D" || pcc.Exports[pcc.Exports.Count - 1].ClassName == "TextureFlipBook")
                {
                    Print("PCC in Game Directory. Texture Found. Adding to DB...");
                    int exportID = (pcc.Exports.Count - 1);
                    Texplorer2 tp = new Texplorer2();
