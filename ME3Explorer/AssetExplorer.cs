...
namespace ME3Explorer
{
    public partial class AssetExplorer : Form
    {
        ...
        private void makeModToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                METexture2D newTex = new METexture2D() { texName = tex2D.texName };
                newTex.PCCs.Add(new PCCEntry(pcc.pccFileName, index));

                KFreonLibME.Scripting.ModMakerHelper.CreateTextureJob(newTex, d.FileName, 3, pathBIOGame);
                MessageBox.Show("Done.");
            }
        }
