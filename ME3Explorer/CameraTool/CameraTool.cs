...
namespace ME3Explorer.CameraTool
{
    public partial class CamTool : Form
    {
        ...
        {
            KFreonLibGeneral.Debugging.DebugOutput.StartDebugger("Main ME3Explorer Form");
            string basepath = KFreonLibME.MEDirectories.ME3Directory.cookedPath;
            string[] files = Directory.GetFiles(basepath, "*.pcc");
            OverView = new List<OverViewStruct>();
            for (int f = 0; f < files.Length; f++)
            {
                string file = files[f];
                KFreonLibGeneral.Debugging.DebugOutput.PrintLn((f + 1) + " / " + files.Length + " : " + file + " :");
                ...
                for (int i = 0; i < p.Exports.Count; i++)
                {
                    if (p.GetObject(p.Exports[i].idxClass) == "InterpData")
                    {
                        ...
                        if (count++ == 0)
                            s = "\n";
                        KFreonLibGeneral.Debugging.DebugOutput.PrintLn(s + "found " + i + " : " + p.GetObjectPath(i + 1) + p.GetObject(i + 1));
                    }
                }
