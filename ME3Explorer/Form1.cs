...
namespace ME3Explorer
{
    public partial class Form1 : Form
    {
        public Form1()
        {
            ProfileOptimization.SetProfileRoot(UsefulThings.General.GetExecutingLoc());
            ProfileOptimization.StartProfile("ME3Explorer_PerformanceProfile");
        ...
        {
            // KFreon: Increment build if necessary (NOTE: Off thread so should call and continue)
            if (Debugger.IsAttached)
                KFreonLibME.Misc.Methods.IncrementVersion();
        ...
        private void texplorerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            System.Windows.Forms.Integration.ElementHost.EnableModelessKeyboardInterop(texplorer);  // KFreon: Needed to make textboxes behave in WPF apps for some reason
            texplorer.Show();
        }
        ...
        {
            MessageBox.Show("Number of threads changed to " + KFreonLibME.Misc.Methods.SetNumThreads(true));
        }
        ...
        {
            String defdir = MEDirectories.GetBIOGameProperty(3) ?? "";
            ...
            MEDirectories.SetBIOGame(3, Path.GetDirectoryName(cookPath));
        ...
        private void modMakerToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            ...
            NewModMaker modmaker = new NewModMaker();
            System.Windows.Forms.Integration.ElementHost.EnableModelessKeyboardInterop(modmaker);  // KFreon: Needed to make textboxes behave in WPF apps for some reason
            modmaker.Show();
        }

        private void tPFDDSToolsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            KFreonTPFToolsV3 tpftools = new KFreonTPFToolsV3();
            System.Windows.Forms.Integration.ElementHost.EnableModelessKeyboardInterop(tpftools);  // KFreon: Needed to make textboxes behave in WPF apps for some reason
            tpftools.Show();
        }
        ...
        {
            ResILWrapper.ResILImageConverter converter = new ResILWrapper.ResILImageConverter();
            converter.Show();
        }
    }
}
